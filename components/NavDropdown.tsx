...
export function NavDropdown() {
  ...
  return (
    <div
      className="group flex items-center overflow-hidden rounded-full bg-white/5 transition hover:bg-[#1a1a1a]"
      role="tablist"
    >
      {/* Globe + selected state */}
      <div className="flex shrink-0 items-center gap-2 px-2 py-1.5 md:px-2.5">
        <Globe className="h-4 w-4 text-white/60 md:h-5 md:w-5" aria-hidden />
      ...
      <div className="flex max-w-0 items-center gap-0.5 overflow-hidden pr-2 transition-[max-width] duration-300 ease-out group-hover:max-w-[120px]">
        {TABS.filter((tab) => pathname !== tab.href).map((tab) => (
          <Link
            ...
            href={tab.href}
            role="tab"
            className="whitespace-nowrap rounded-full px-3 py-1.5 text-base font-medium text-white/80 transition hover:bg-white/10 hover:text-white md:text-lg"
          >
            {tab.label}
          </Link>
        ))}
